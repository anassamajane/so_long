...
int	init_game(t_game *game)
{
	...
	game->mlx = mlx_init();
	game->mlx_window = mlx_new_window(game->mlx, game->map_width * TILE_SIZE, game->map_height * TILE_SIZE, "Dragon Ball");
	...
	if (load_player_images(game) || load_collectibles_images(game) || load_door_images(game))
		return (1);
	game->collected = 0;
	game->step_counter = 0;
